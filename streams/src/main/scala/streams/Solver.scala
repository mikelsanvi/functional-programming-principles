...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] = {
    val (block, movements) = initial.head
    val neighbors = newNeighborsOnly(neighborsWithHistory(block,movements),explored)
    neighbors #::: from(initial.tail #::: neighbors,explored + block)
  }
