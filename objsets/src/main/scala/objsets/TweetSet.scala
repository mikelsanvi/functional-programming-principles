...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val filtered = right.filterAcc(p, left.filterAcc(p,acc))
    if(p(elem)) {
      filtered.incl(elem)
    } else {
      filtered
  ...
  def mostRetweeted: Tweet = {
    val leftMostRewteeted = if(left.isInstanceOf[NonEmpty]) {
      ...
      elem
    val rightMostRewteeted = if(right.isInstanceOf[NonEmpty]) {
    ...
    if(leftMostRewteeted.retweets > rightMostRewteeted.retweets)
      leftMostRewteeted
    else
      rightMostRewteeted
  }
  ...
  override def toString: String = {
    "("+left.toString+"."+elem.text+"."+right.toString+")"
  }
}
